    ...
    function check_user($username, $password){
      ...
      function send_email($email){
      	...
      	if($sql_email->fetchColumn() > 0){
      		...
      		$content = "Follow this link to reset your password: \n " . "https://".$_SERVER['HTTP_HOST']."/~tmalmst/CT310-P2/passwordreset.php"."?key=$key"; // had to change the absolute path when I removed one of the unessecary folders.
      		// echo "<br>\n";
      		// echo $content;
      ...
      function search_animal($animal){
          ...
          if($animal_stmt->fetchColumn() > 0){
             // echo "exists! <br>";
             $res = $this->query("SELECT pet_name FROM animals WHERE pet_name = '$first_letter'");
             return $res->fetchColumn();
          } else {
             // echo "non existant <br>";
             return null; // negate top statement.
